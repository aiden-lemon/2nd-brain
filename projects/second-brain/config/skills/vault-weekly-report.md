...
- 기본: 보고일 포함 지난 7일. `--since="<보고일-7일> 00:00"`.
- 사용자가 기간을 지정하면 그 기간을 쓰고, 보고서 frontmatter의
  `period-start`/`period-end`에 그대로 기록한다.

## 집계 절차 (전수 통계)

경계 커밋을 먼저 잡는다: `BASE=$(git rev-list -1 --before="<period-start> 00:00" master)`.

| 항목 | 명령 |
| --- | --- |
| 커밋 수 (전체 / 실작업) | `git log --since=... --oneline \| wc -l` / `--no-merges` 동일 |
| 머지된 PR 수 | `git log --since=... --merges --pretty="%s" \| grep -c "Merge pull request"` |
| 기여자별 커밋 | `git log --since=... --no-merges --pretty="%an" \| sort \| uniq -c \| sort -rn` |
...
| 일별 커밋 | `git log --since=... --pretty="%ad" --date=format:"%m-%d" \| sort \| uniq -c` |
...
진행한다 (MCP 서버 등록·scope 재동의 절차는 각 팀의 MCP 설치 문서를 따른다).
