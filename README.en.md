...
| **Word ingest** | Converts Word documents (.doc/.docx) to Markdown, preserving heading and table structure, and drops them into `Clippings/` (`doc2md-ingest`) |
...
| `doc2md-ingest` | Converts `.doc`/`.docx` to Markdown and drops it into `Clippings/` — pandoc directly for `.docx`, with LibreOffice, Word COM, and textutil fallbacks for `.doc`; an out-of-vault mode handles documents that must not be committed |
